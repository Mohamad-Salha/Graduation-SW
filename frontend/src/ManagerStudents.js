...
const ManagerStudents = () => {
  ...
  const loadStudentsData = () => {
    ...
    setNewStudents(pendingStudents);
    ...
    const theoretical = allStudents.filter(
      (student) =>
        student.approved &&
        (student.status === 'theoretical' || !student.status)
    );

    const practical = allStudents.filter(
      (student) => student.approved && student.status === 'practical'
    );

    const exam = allStudents.filter(
      (student) => student.approved && student.status === 'exam'
    );
  ...
  const handleApproveStudent = (studentId) => {
    ...
    if (studentToApprove) {
      ...
      allStudents.push(approvedStudent);
  ...
  const handleRejectStudent = (studentId) => {
    const pendingStudents = JSON.parse(localStorage.getItem('pendingStudents') || '[]');
    const studentToReject = pendingStudents.find((student) => student.id === studentId);
    const updatedPending = pendingStudents.filter((student) => student.id !== studentId);

    localStorage.setItem('pendingStudents', JSON.stringify(updatedPending));
    loadStudentsData();

    alert(`❌ Student ${studentToReject?.name} application has been rejected.`);
  };
  ...
  const handleGoToExam = (studentId) => {
    ...
    if (student) {
      ...
      alert(`🎯 ${student.name} has been moved to Theoretical Exams section!`);
    }
  };
  ...
  const handlePassExam = (studentId) => {
    ...
    alert(`✅ ${student.name} has PASSED the theoretical exam and moved to PRACTICAL section.`);
  };
    ...
    <div className="manager-students">
      <div className="container">
        ...
        <div className="tabs-navigation">
          ...
          <button
            className={`tab-btn ${activeTab === 'theoretical' ? 'active' : ''}`}
              ...
              {newStudents.length > 0 ? (
                <div className="students-grid">
                  {newStudents.map((student) => (
                    <div key={student.id} className="student-card new-student">
                      <div className="student-header">
                        ...
                        <div className="student-info">
                          ...
                          <p className="apply-date">
                            📅 Applied:{' '}
                            {new Date(student.applyDate).toLocaleDateString()}
                          </p>
                ...
                <div className="no-students">
                  ...
                  <p>There are no pending student applications at the moment.</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="tab-content">
              ...
              {theoreticalStudents.length > 0 ? (
                    ...
                    return (
                      <div
                        ...
                        className={`student-card ${
                          student.readyForExam ? 'ready-student' : 'theoretical-student'
                        }`}
                      >
                        <div className="student-header">
                          <div className="student-avatar">👨‍🎓</div>
                          <div className="student-info">
                            ...
                            <div className="progress-info">
                              ...
                              <span>
                                ✅ Passes: {passStatus.passedCount}/{passStatus.total}
                              </span>
                            </div>
                            <div className="student-status">
                        ...
                        <div className="student-details">
                          <div className="detail-item">
                            ...
                            <span className={student.trafficSigns ? 'passed' : 'pending'}>
                              {student.trafficSigns ? '✅ Passed' : '❌ Pending'}
                            </span>
                          </div>
                          <div className="detail-item">
                            <span>📜 Traffic Laws:</span>
                            <span className={student.trafficLaws ? 'passed' : 'pending'}>
                              {student.trafficLaws ? '✅ Passed' : '❌ Pending'}
                            </span>
                          </div>
                          <div className="detail-item">
                            <span>🔧 Car Mechanics:</span>
                            <span className={student.carMechanics ? 'passed' : 'pending'}>
                              {student.carMechanics ? '✅ Passed' : '❌ Pending'}
                            </span>
                          </div>
                          <div className="detail-item">
                            <span>📝 Final Quiz:</span>
                            <span className={student.quiz ? 'passed' : 'pending'}>
                              {student.quiz ? '✅ Passed' : '❌ Pending'}
                            </span>
                          </div>
                        </div>
                        {student.readyForExam && (
                          <div className="student-actions">
                            <button
                              className="exam-btn"
                              onClick={() => handleGoToExam(student.id)}
                            >
                              🎯 Go to Exam
                            </button>
                          </div>
                ...
                <div className="no-students">
                  ...
                  <p>There are no students in the theoretical phase at the moment.</p>
                </div>
              )}
            </div>
          )}
              ...
              {practicalStudents.length > 0 ? (
                <div className="students-grid">
                  {practicalStudents.map((student) => {
                    const lessons = student.practicalLessons || 0;
                    const paid = student.practicalPaid || 0;
                    const total = lessons * LESSON_PRICE;
                    const remaining = total - paid;

                    return (
                      <div key={student.id} className="student-card practical-student">
                        <div className="student-header">
                          <div className="student-avatar">🚗</div>
                          <div className="student-info">
                            <h3>{student.name}</h3>
                            ...
                            <p className="student-instructor">
                              👨‍🏫 Trainer:{' '}
                              {student.practicalTrainer || 'Not selected by student yet'}
                            </p>
                            <div className="progress-info">
                              <span>📅 Practical Lessons: {lessons}</span>
                            </div>
                          </div>
                        </div>
                        <div className="student-details">
                          <div className="detail-item">
                            <span>💰 Total Amount (so far):</span>
                            <span>{total} ₪</span>
                          </div>
                          <div className="detail-item">
                            <span>✅ Paid:</span>
                ...
                <div className="no-students">
                  ...
                  <p>Students will appear here after passing the theoretical exam.</p>
                </div>
              )}
            </div>
          )}
              ...
              {examStudents.length > 0 ? (
                <div className="students-grid">
                  {examStudents.map((student) => {
                    ...
                    return (
                      <div key={student.id} className="student-card exam-student">
                        <div className="student-header">
                          <div className="student-avatar">🎓</div>
                          <div className="student-info">
                            <h3>{student.name}</h3>
                            ...
                            <div className="student-status">
                              {student.examPassed ? (
                                <span className="status-badge ready">✅ Exam Passed</span>
                              ) : (
                                <>
                        ...
                        <div className="student-actions">
                          <button
                            className="approve-btn"
                            onClick={() => handlePassExam(student.id)}
                            disabled={student.examPassed}
                          >
                            {student.examPassed
                ...
                <div className="no-students">
                  ...
                  <p>There are no students scheduled for exams at the moment.</p>
                </div>
              )}
            </div>
          )}
