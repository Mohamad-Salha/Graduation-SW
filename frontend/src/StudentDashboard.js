...
const StudentDashboard = () => {
	...
	useEffect(() => {
		...
		if (currentStudent) {
			...
			if (
				...
				currentStudent.course !== "Not Selected Yet"
			) {
	...
	const loadStudentData = async () => {
		...
		try {
			...
			const profile = await api.student.getProfile(token);
			console.log("Student profile:", profile);
			setProfileData(profile);

			// Update state based on profile
			if (profile.license) {
				setSelectedCourse(profile.license.name);
				setHasSelectedCourse(true);
			}
			...
			if (profile.theoPassed) {
				setHasPassedExam(true);
				setStudentStatus("practical");
			}

			if (profile.trainer) {
				setPracticalTrainer(profile.trainer.name);
				setStudentStatus("practical");
			}

			// Load available courses if student hasn't enrolled yet
			if (!profile.license) {
				await loadCourses();
			} else if (profile.license && !profile.teacher) {
				// If enrolled but no teacher, load teachers
				await loadTeachers();
			} else if (profile.theoPassed && !profile.trainer) {
				// If passed theoretical exam but no trainer, show trainer selection
				setShowTrainerSelection(true);
				await loadTrainers();
			}
		} catch (error) {
	...
	const loadCourses = async () => {
		...
		try {
			const data = await api.student.getLicenses(token);
			console.log("Licenses data:", data);

			// Map backend licenses to frontend course format
			const mappedCourses = data.licenses.map((license) => ({
				id: license._id,
				type: license.name,
				icon: getIconForCourse(license.name),
				color: getColorForCourse(license.name),
				description:
					license.description || `Learn to drive ${license.name}`,
				price: license.price,
				minSessions: license.minPracticalSessions,
			}));
	...
	const getIconForCourse = (courseName) => {
		if (courseName.includes("Manual")) return "🚗";
		if (courseName.includes("Automatic")) return "🚙";
		if (courseName.includes("Motorcycle")) return "🏍️";
		if (courseName.includes("Light Truck")) return "🚐";
		if (courseName.includes("Heavy Truck")) return "🚛";
		if (courseName.includes("Trailer")) return "🚚";
		if (courseName.includes("Bus")) return "🚌";
		return "🚗";
	};

	const getColorForCourse = (courseName) => {
		if (courseName.includes("Manual")) return "#6366f1";
		if (courseName.includes("Automatic")) return "#10b981";
		if (courseName.includes("Motorcycle")) return "#f59e0b";
		if (courseName.includes("Light Truck")) return "#ec4899";
		if (courseName.includes("Heavy Truck")) return "#8b5cf6";
		if (courseName.includes("Trailer")) return "#06b6d4";
		if (courseName.includes("Bus")) return "#dc2626";
		return "#6366f1";
	};
	...
	const loadTeachers = async () => {
		...
		try {
			const data = await api.student.getTeachers(token);
			console.log("Teachers data:", data);

			// Map backend teachers to frontend format
			const mappedTeachers = data.teachers.map((teacher, index) => ({
			...
			setTeachers(getDefaultTeachers());
		} finally {
	...
	const loadTrainers = async () => {
		...
		try {
			const data = await api.student.getTrainers(token);
			console.log("Trainers data:", data);

			// Map backend trainers to frontend format
			const mappedTrainers = data.trainers.map((trainer, index) => ({
				trainerId: trainer.trainerId,
				name: trainer.name,
				email: trainer.email,
				phone: trainer.phone,
				icon: "🚗",
				color: ["#1d4ed8", "#059669", "#7c3aed", "#dc2626"][index % 4],
				specialties: ["All Vehicle Types"], // Would come from backend in full implementation
				experience: "10 years",
				assignedStudentsCount: trainer.assignedStudentsCount,
			}));

			setTrainers(mappedTrainers);
		} catch (error) {
			console.error("Error loading trainers:", error);
			// Fallback to hardcoded trainers if API fails
	...
	const getDefaultTrainers = () => [
		{
			...
			icon: "🚗",
			color: "#1d4ed8",
			experience: "10 سنوات (سيارات وشحن)",
		},
		{
			name: "خالد",
			...
			icon: "🏍️",
			color: "#059669",
			experience: "8 سنوات (دراجات نارية)",
		},
		{
			name: "محمود",
			specialties: ["Heavy Truck", "Bus"],
			icon: "🚛",
			color: "#7c3aed",
			experience: "12 سنة (شاحنات ثقيلة)",
		},
		{
			name: "فهد",
			specialties: ["Bus", "Trailer", "Heavy Truck"],
			icon: "🚌",
			color: "#dc2626",
	...
	// Initialize with default courses, will be replaced by API data
	...
	const getEligibleTrainers = () => {
		...
		// If we have trainers from API, use them (they're all eligible for now)
		if (trainers.length > 0) {
			return trainers;
		}
		// Otherwise use hardcoded trainers filtered by specialty
		return practicalTrainers.filter((t) =>
			t.specialties.includes(selectedCourse)
		);
	};
	...
	const handleCourseSelect = async (course) => {
		...
		try {
			...
			const result = await api.student.enrollInCourse(token, course.id);
			console.log("Enrollment result:", result);

			// Update local state
			setSelectedCourse(course.type);
			setHasSelectedCourse(true);

			// Load teachers for next step
			await loadTeachers();
			setShowInstructors(true);

			alert(`Successfully enrolled in ${course.type}!`);
		} catch (error) {
	...
	const handleInstructorSelect = async (instructor) => {
		...
		// Check if this is API-based teacher (has teacherId)
		if (instructor.teacherId) {
			try {
				// Call API to choose teacher
				const result = await api.student.chooseTeacher(
					token,
					instructor.teacherId
				);
				console.log("Teacher selection result:", result);
				...
				alert(
					`Successfully chose ${instructor.name} as your theoretical teacher!`
				);
				return;
			} catch (error) {
				...
				alert(
					error.message ||
						"Failed to choose teacher. Please try again."
				);
				return;
			}
		}

		// Fallback to localStorage for old method
		const currentStudentData = JSON.parse(
			localStorage.getItem("currentStudent") || "{}"
		);
		...
		localStorage.setItem(
			`selectedCourse_${studentUsername}`,
			selectedCourse
		);
		localStorage.setItem(
		...
		alert(
			`✅ Successfully selected: ${selectedCourse} with Instructor: ${instructor.name}`
		);
	};
	...
	const handlePracticalClick = () => {
		if (!hasSelectedCourse) {
			alert(
				"Please select a theoretical course first from the Courses section"
			);
			return;
		}
		setShowSchedule(true);
	...
	const handlePracticalPanelClick = () => {
		if (!hasPassedExam) {
			alert(
				"يجب أن تجتاز الامتحان النظري أولاً قبل البدء بالمرحلة العملية."
			);
			return;
		}
	...
	const handlePracticalTrainerSelect = async (trainer) => {
		const token = getAuthToken();

		// If we have API token and trainer has ID, use API
		if (token && trainer.trainerId) {
			try {
				const result = await api.student.chooseTrainer(
					token,
					trainer.trainerId
				);
				console.log("Trainer selection result:", result);
				...
				alert(
					`✅ تم اختيار المدرب ${trainer.name} للتدريب العملي الخاص بك`
				);
				return;
			} catch (error) {
				console.error("Error choosing trainer:", error);
				alert(`❌ ${error.message || "Failed to choose trainer"}`);
				return;
			}
		}
					...
					<div className="glass-card">
							...
							<div className="header-content">
								<div className="title-section">
									<h1 className="dashboard-title">
										اختيار المدرب العملي
									</h1>
									<p className="dashboard-subtitle">
										لقد اجتزت الامتحان النظري ✅ اختر الآن
										المدرب العملي المناسب لنوع رخصتك.
									</p>
								</div>
						...
						{selectedCourse ? (
								...
								{eligible.length > 0 ? (
									eligible.map((trainer, index) => (
										...
										>
											...
											<div className="select-instructor-btn">
												اختر هذا المدرب
											</div>
										</div>
									))
									...
									<div className="no-students">
										...
										<h3>
											لا يوجد مدربون مناسبون لهذا النوع من
											الرخصة حالياً
										</h3>
							...
							<div className="no-students">
								...
								<p>
									اذهب أولاً إلى My Courses واختر نوع الرخصة.
								</p>
							</div>
						)}
						...
						<div className="action-buttons">
							...
							<button
								className="logout-button glass-button"
						...
						<div className="dashboard-header">
							<div className="header-content">
								<div className="title-section">
									...
									<p className="dashboard-subtitle">
										Select the best instructor for your{" "}
						...
						<div className="glass-card">
							...
							<div className="course-details-card">
								<div
									...
									style={{ color: getSelectedCourseColor() }}
								>
									{getSelectedCourseIcon()}
								</div>
								<div className="course-info">
									...
									{hasPassedExam && (
										<div className="exam-passed-banner">
											🎉 You have passed the theoretical
											exam. Practical training is now
											available.
										</div>
									)}
								</div>
						...
						<div className="dashboard-header">
							<div className="header-content">
								<div className="title-section">
									...
									<p className="dashboard-subtitle">
										Select the vehicle you want to learn
										driving
									</p>
								</div>
								<div className="progress-steps">
						...
						<div className="dashboard-header">
							<div className="header-content">
								<div className="title-section">
									...
									<p className="dashboard-subtitle">
										Track your practical lessons, payments,
										and trainer information
									</p>
								</div>
								<div className="user-badge">
									<span className="user-avatar">🚗</span>
						...
						<div className="selected-info-grid">
							<div className="info-card gradient-card">
								...
								<div className="info-content">
									<h3>Trainer</h3>
									<p>
										{practicalTrainer || "Not selected yet"}
									</p>
								</div>
							</div>
							<div className="info-card gradient-card">
							...
							<div className="timeline">
								...
								<div className="timeline-item">
									...
									<div className="timeline-content">
										<h4>Remaining</h4>
										<p>
											after payments recorded by manager
										</p>
										<span className="timeline-time">
											{practicalRemaining} ₪
										</span>
					...
					<div className="dashboard-header">
						<div className="header-content">
							<div className="title-section">
								...
								<p className="dashboard-subtitle">
									Welcome back, {studentName}! Ready to
									continue your driving journey?
								</p>
							</div>
							<div className="user-badge">
								<span className="user-avatar">👨‍🎓</span>
					...
					<div className="dashboard-actions">
						...
						>
							...
							<div className="action-content">
								...
								<p>
									View your theoretical training timetable and
									sessions
								</p>
							</div>
