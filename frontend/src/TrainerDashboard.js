...
const TrainerDashboard = () => {
  ...
  const handleRequestCar = (slotId) => {
    ...
    if (!slot.studentId || !slot.vehicleType) {
      alert('You must assign a student and course to this slot before requesting a car.');
      return;
    }
    ...
    if (existing) {
      alert('You already requested a car for this lesson. Please wait for manager approval.');
      return;
    }
  ...
  const handleCompleteLesson = (slotId) => {
    ...
    if (slot.status !== 'in-progress' && slot.status !== 'booked' && slot.status !== 'waiting-car') {
    ...
    if (slot.carId) {
      ...
      const updatedCars = cars.map((c) =>
        c.id === slot.carId
          ? { ...c, busy: false, currentTrainer: null, currentStudent: null }
          : c
      );
    ...
    alert(`✅ Lesson completed. Student balance updated (+${lessonPrice}₪).`);
  };
  ...
  if (!currentTrainer) {
    return <div className="loading-screen">Loading trainer dashboard...</div>;
  }
          ...
          <section className="students-section">
            ...
            {students.length === 0 ? (
              <div className="empty-state">
                ...
                <p>Students who select you as a practical trainer will appear here.</p>
              </div>
            ) : (
                ...
                {students.map((student) => (
                  <div
                    ...
                    className={`student-card ${
                      student.practicalLessons > 0 ? 'student-card-active' : ''
                    }`}
                    onClick={() => handleSelectStudentInfo(student)}
                  >
                    <div className="student-header">
                      ...
                      <div className="student-main-info">
                        ...
                        <p className="student-lessons">
                          Lessons: {student.practicalLessons || 0}
                        </p>
                        <p className="student-paid">
                          Paid: {student.practicalPaid || 0} ₪
                        </p>
                      </div>
                    </div>
                    <div className="student-footer">
                      <span className="student-status">
                        Status: {student.status || 'practical'}
                      </span>
                    </div>
                  </div>
                ))}
            ...
            {selectedStudent && (
              <div className="student-details-panel">
                ...
                <p>
                  <strong>Lessons:</strong> {selectedStudent.practicalLessons || 0}
                </p>
                <p>
                  <strong>Paid:</strong> {selectedStudent.practicalPaid || 0} ₪
                </p>
              </div>
            )}
          </section>
            ...
            <form className="add-slot-form" onSubmit={handleAddSlot}>
              <div className="form-row">
                ...
                <div className="form-field button-field">
                  <button type="submit" className="add-slot-btn">
                    + Add Lesson
                  </button>
                </div>
              </div>
                  ...
                  <tbody>
                    {getSlotsForSelectedDate().map((slot) => (
                      <tr key={slot.id}>
                        ...
                        <td>{slot.carId ? `Car #${slot.carId}` : '-'}</td>
                       <td className="actions-cell">

  {/* << زر Request Car هنا */}
  {(() => {
    if (!slot.studentId) return null;
    ...
    return (
      <button
        ...
        onClick={() => handleRequestCar(slot.id)}
      >
        🚙 Request Car
      </button>
    );
